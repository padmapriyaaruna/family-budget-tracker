...
def show_member_expense_tracking(user_id):
    ...
    with tab2:
        ...
        with col1:
            ...
            # Add Previous Allocation button
            if st.button("📋 Add Previous Allocation", use_container_width=True, key="btn_add_prev_alloc"):
                st.session_state.show_previous_allocations = True
            
            # Show previous allocations modal
            if st.session_state.get('show_previous_allocations', False):
                with st.expander("📋 Select Previous Allocations", expanded=True):
                    # Get past allocations
                    past_allocations = db.get_past_allocations(
                        user_id, 
                        st.session_state.budget_year, 
                        st.session_state.budget_month
                    )
                    
                    if not past_allocations:
                        st.info("No previous allocations found in the last 6 months")
                        if st.button("Close", key="close_no_prev"):
                            st.session_state.show_previous_allocations = False
                            st.rerun()
                    else:
                        st.caption(f"Found {len(past_allocations)} allocation categories from past 6 months")
                        
                        # Create multiselect options
                        allocation_options = {}
                        for alloc in past_allocations:
                            label = f"{alloc['category']} - {config.CURRENCY_SYMBOL}{alloc['amount']:,.2f}"
                            allocation_options[label] = alloc
                        
                        # Multiselect widget
                        selected_labels = st.multiselect(
                            "Select allocations to add:",
                            options=list(allocation_options.keys()),
                            help="Choose one or more allocations to copy to current period",
                            key="multiselect_prev_alloc"
                        )
                        
                        if selected_labels:
                            # Calculate projected total
                            selected_total = sum(allocation_options[label]['amount'] for label in selected_labels)
                            projected_total = total_allocated + selected_total
                            
                            # Display projection
                            st.info(f"**Projected Total**: {config.CURRENCY_SYMBOL}{projected_total:,.2f}")
                            st.caption(f"Current: {config.CURRENCY_SYMBOL}{total_allocated:,.2f} + Selected: {config.CURRENCY_SYMBOL}{selected_total:,.2f}")
                            
                            # Validation
                            if projected_total > total_income:
                                st.error(f"⚠️ **Selection is not valid as it exceeds the provided income.**\n\n"
                                        f"Total Income: {config.CURRENCY_SYMBOL}{total_income:,.2f}\n\n"
                                        f"Projected Total: {config.CURRENCY_SYMBOL}{projected_total:,.2f}\n\n"
                                        f"Exceeds by: {config.CURRENCY_SYMBOL}{projected_total - total_income:,.2f}")
                                if st.button("Cancel", key="cancel_exceed", use_container_width=True):
                                    st.session_state.show_previous_allocations = False
                                    st.rerun()
                            else:
                                # Valid selection - show add button
                                col_add, col_cancel = st.columns(2)
                                with col_add:
                                    if st.button("✅ Add Selected", key="add_prev_alloc_btn", use_container_width=True):
                                        # Add each selected allocation
                                        added_count = 0
                                        failed_count = 0
                                        for label in selected_labels:
                                            alloc = allocation_options[label]
                                            if db.add_allocation(user_id, alloc['category'], alloc['amount'], 
                                                                st.session_state.budget_year, st.session_state.budget_month):
                                                added_count += 1
                                            else:
                                                failed_count += 1
                                        
                                        if added_count > 0:
                                            st.success(f"✅ Added {added_count} allocation(s) successfully!")
                                        if failed_count > 0:
                                            st.warning(f"⚠️ {failed_count} allocation(s) already exist")
        ...
        with col1:
            ...
            if not categories:
                ...
                with st.form("expense_form", clear_on_submit=True):
                    ...
                    subcategory_options = ["Investment", "Food - Online", "Food - Hotel", "Grocery - Online", "Grocery - Offline", "School Fee", "Others"]
